...
impl Writer {
    ...
    pub fn write_byte(&mut self, byte: u8) { self.write_byte_color(byte, self.color_code) }
    ...
    pub fn write_string_color(&mut self, s: &str, color: ColorCode) {
        for byte in s.bytes() {
            match byte {
                ...
                _ => self.write_byte_color(0xfe, color),
            }
        }
    }

    pub fn write_string(&mut self, s: &str) { self.write_string_color(s,self.color_code); }
    ...
    fn new_line(&mut self, color: ColorCode) {
        ...
        self.clear_row(BUFFER_HEIGHT - 1, color);
